...
import { addProduct, getCategoriesByClient, getClients, getProductsByClient } from '@/lib/api';
...
export default function ProductsPage() {
  ...
  return (
    <div className="p-6 max-w-xl">
      ...
      <select className="border p-2 w-full mb-4" value={clientId} onChange={e => setClientId(e.target.value)}>
        <option value="">Select Client</option>
        {clients?.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
      </select>

      {clientId && (
        <form onSubmit={handleSubmit(data => mutation.mutate({ ...data, clientId }))} className="space-y-4">
      ...
      <ul className="mt-2 space-y-2">
        {products?.map(prod => <li key={prod.id} className="border p-2">{prod.name} - ${prod.price}</li>)}
      </ul>
    </div>
  );
}